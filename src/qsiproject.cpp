...
QSIProject::QSIProject(QString path, QObject *parent) : QObject(parent) {
	...
	if(suffix == "ssproj") {
		this->readSSProj(projectFile);
	} else if(filename == "Cellscript.mjs" || filename == "Cellscript.js") {
		this->readCellscript(projectFile);
	} else if(suffix == "sgm") {
		this->readSGM(projectFile);
	}
...
bool QSIProject::readSSProj(QFile* projectFile) {
	...
	while(!stream.atEnd()) {
		...
		if(key == "author") this->author = value;
			...
			if(value == "Cell") this->compiler = QSIProject::Cell;
			this->compiler = value;
		}
...
bool QSIProject::readSGM(QFile *projectFile) {
	...
	this->compiler = QSIProject::Vanilla;
	return true;
}
